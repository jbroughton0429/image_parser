...
3. Build the two packer images
4. Edit TF Vars files: 
   ...
   - terraform/platform/main.tf (ami - Same as devops, however the image for this one is labeled 'database')
      = If you are in Prod Environment, switch Tags here to Prod from Dev
      = Change the AMI to that of the DevOps Image built from Packer (located in your AWS AMI Console)
5. If this is your first time building TF in this environment, uncomment the 'bucket' line in:
   'deploy.sh'
6. Execute 'deploy.sh' to build the TF environment
7. Your 'Dev' Environment is no longer needed, log into 'Console' for all further steps.
